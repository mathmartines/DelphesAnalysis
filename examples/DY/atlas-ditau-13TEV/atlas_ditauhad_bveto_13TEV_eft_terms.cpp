...
int main () {
    ...
    const string simulations_folders = "/home/martines/work/MG5_aMC_v3_1_1/PhD/High-PT/atlas-ditau-13TEV/SMEFT/";
    
    /// EFT terms
    vector<string> eft_terms = {"C1lq", "C3lq", "C1lq-C1lq", "C1lq-C3lq", "C3lq-C3lq"};
    ...
    for (const auto& term: eft_terms) {
        ...
        for (int i = 1; i <= nbins; i++) {
            ...
            TString root_file = simulations_folders + "root_files_3322/atlas-ditau-" + term + "-" + to_string(i) + ".root";

            /// path to the banner that stores the cross-section
            string bannerfile = simulations_folders + "lhe_files_3322/atlas-ditau-" + term + "-" + to_string(i) + ".lhe";
    ...
    /// Saves .json file
    // save json file
    ofstream file("atlas-ditau-bveto-13TEV-3322.json");
    if (file.is_open()) {
